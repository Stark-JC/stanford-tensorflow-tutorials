...
SKIP_STEP = 5000  # output loss every SKIP_STEP times
...
class Word2Vec(object):
    def __init__(self, vocab_size, embed_size, num_sampled=NUM_SAMPLED):
        ...
        self.nce_weight = tfe.Variable(tf.truncated_normal([vocab_size, embed_size],
    ...
    def save(self, dataset, save_path):
        checkpoint = tfe.Checkpoint(embed_matrix=self.embed_matrix)
    ...
    def visualize(self, visual_fld, num_visualize):
        ...
        summary_writer = tf.contrib.summary.create_file_writer(
            os.path.join(visual_fld, 'summary'), flush_millis=10000)

        with summary_writer.as_default(), tf.contrib.summary.always_record_summaries():
            tf.contrib.summary.scalar("embed_matrix", self.embed_matrix)
...
def main():
    ...
    while num_train_steps < NUM_TRAIN_STEPS:
        for center_words, target_words in tfe.Iterator(dataset):
            ...
            loss_batch, grad = grad_fn(center_words, target_words)
